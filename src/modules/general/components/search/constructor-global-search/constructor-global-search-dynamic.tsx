import { Box, List, ListItem, Stack, Text, useToken } from '@chakra-ui/react'
import {
  Product,
  Section,
  useCioAutocomplete,
...
import { CONSTRUCTOR_DEFAULT_CATEGORY } from '@modules/product/pages/product-listing-shared'
...
export const ConstructorGlobalSearch = ({ userId }: { userId?: string }) => {
  ...
  const hits = useMemo(() => {
    const rawHits =
      sections.find(
        (section: any) =>
          section.indexSectionName === CONSTRUCTOR_CONSTANT.SECTION_ID.PRODUCTS
      )?.data ?? []
    return rawHits.map((item) =>
      transformConstructorHit({ item: item as Product, currency })
    )
  }, [sections])

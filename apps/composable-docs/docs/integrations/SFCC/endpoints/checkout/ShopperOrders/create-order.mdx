...
This endpoint is used to convert a customer basket into an Order.
...
<Tabs>
  <TabItem value="commerce-sdk" label="Commerce SDK">
    ```js
    const body = {
      basketId: 'a10ff320829cb0eef93ca5310a', // required
    }
    const parameters = {
      locale: 'en_CA',
    }
    const order = await shopperOrdersClient.createOrder({ body, parameters });
    ```
  </TabItem>
  <TabItem value="scapi" label="SCAPI">
    ```http
    POST /checkout/shopper-orders/v1/organizations/{organizationId}/orders?siteId=RefArch HTTP/1.1
...
`POST https://{shortCode}.api.commercecloud.salesforce.com/checkout/shopper-orders/{version}/organizations/{organizationId}/orders?siteId=RefArch`
...
<Tabs>
  ...
  Success, returns the created order
  ```json
  {
  ...
  ```
  #### Reasons for Failure
  - Gift certificate line item creation failure
  - Missing or invalid Basket ID
  - Invalid product
  - Invalid product option
  - Product unavailable
  - Current customer does not match basket customer
  - Basket contains flashes
  </TabItem>
...
- commerce-sdk: [Checkout.ShopperOrders.createOrder](https://salesforcecommercecloud.github.io/commerce-sdk/classes/checkout.shopperorders.html#createorder)
- SCAPI: [createOrder](https://developer.salesforce.com/docs/commerce/commerce-api/references/shopper-orders?meta=createOrder)
